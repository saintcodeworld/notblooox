...
export class Game {
  ...
  private constructor(gameContainerRef: MutableRefObject<any>, websocketUrl?: string, port?: number) {
    ...
    this.websocketManager = new WebSocketManager(this, websocketUrl, port)
  ...
  static getInstance(websocketUrl?: string, port?: number, gameContainerRef?: MutableRefObject<any>): Game {
    if (!Game.instance) {
      if (!gameContainerRef) {
        throw new Error('Game instance not initialized with gameContainerRef')
      }
      Game.instance = new Game(gameContainerRef, websocketUrl, port)
    }
